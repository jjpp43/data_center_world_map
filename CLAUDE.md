...
8. ✅ **Monetization (5b)** — split into 5b.1 (auth foundation) and 5b.2 (Polar.sh wiring).
   - 5b.1 ✅ **Auth + API keys + middleware + rate limits**: Supabase Auth (GitHub OAuth), `/login`, `/auth/callback`, `/dashboard/keys` (create/revoke + per-key monthly quota gauges), root `middleware.ts` on `/api/v1/*` doing bearer validation via `validate_and_charge_api_key` RPC and anonymous IP throttle via `charge_anonymous` RPC. Postgres-based rate limiting (no Upstash dependency). `X-RateLimit-Tier`/`-Limit`/`-Remaining` headers on every response. **Tiers (all monthly, set in migration 0009)**: anonymous 500/mo per IP · free 500/mo · pro 10k/mo ($10.99) · team 50k/mo ($49.99) · enterprise 5M/mo. Monthly across all tiers (incl. anonymous) since the atlas isn't live data — edge cache covers most duplicate hits.
